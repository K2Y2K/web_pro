...
<script type="text/javascript">
	//隐藏答案或显示答案 没成功？ 
	function show() {
		var result = document.getElementById("st_r");
		var resultItem
		result.getElementsByTagName("st_result");
		for (var i = 0; i < resultItem.length; i++) {
			resultItem[i].onclick = function() {
				if (document.all["st_result"].style.display == "block") {
					document.all["st_result"].style.display = "none";
				} else if (document.all["st_result"].style.display = "none") {
...
<body>
	...
	<div id="section1">
		...
		<br><label>出题人：<%=t.getName() %></label><br><label>考生：</label>
		</div>
		<hr class="kj_tab">		
		<div class="kj_1">
			<div class="kj_2">
			...
			<div class="kj_xx">
				...
				<label	class="kj_xx1"><input type="button" onclick="show()" value="答案"> </label>
			</div>
			<div class="kj_xx">
				<input type="radio" value="<%=java_dxt.getJava_dxt_opt2() %>" name="<%=java_dxt.getJava_dxt_id() %>">B:<%=java_dxt.getJava_dxt_opt2() %>
				<label class="kj_xx1"><input id="st_r" type="text"
					name="st_result" value="<%=java_dxt.getJava_dxt_result() %>" style="display: block;"> </label>
			</div>
			<div class="kj_xx">
			...
			<div class="kj_xx">
                ...
                <label class="kj_xx1"><input id="st_r" type="text"
					name="st_result" value="<%=java_dxts.getJava_dxts_result() %>" style="display: block;"> </label>
			</div>
			<div class="kj_xx">
			...
			Java_stDAO j_stdao =new Java_pdtDAO();	
				if (kj.getKj_pdt_id() != null) {
					...
					for (int i = 0; i < arr.length; i++,count3++) {
						if(j_stdao.getJava_stById(Integer.parseInt(arr[i]))!=null){
			...
			<div class="kj_xx">
				<input type="radio" value="0" name="<%=java_pdt.getJava_pdt_id() %>">错误
                 ...
                 <label class="kj_xx1"><input id="st_r" type="text"
					name="st_result" value="<%=result%>" style="display: block;"> </label>
			</div><% }}}%>
